...
# ============ 1. 基本配置 (Hybrid-ANN-DRLSTM - 可调 Kernel) ============
SEED = 39
...
SAVE_BEST_PATH = "best_hybrid_ann_dr_lstm_kfold_model3.txt"
RESULTS_CSV = "grid_results_hybrid_ann_dr_lstm_kfold3.csv"
PLOT_DATA_FILE = "plotting_data_hybrid_ann_dr_lstm_kfold3.txt"
# 新增文件: 记录所有配置在独立测试集上的详细绘图数据
ALL_TEST_PLOT_DATA_FILE = "all_test_plotting_data_hybrid_ann_dr_lstm_kfold3.txt"

# ============ 2. GRID_CONFIGS (已修改: widths=[128, 64], lstm_hidden_size=64) ============
GRID_CONFIGS = [

    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [2, 4], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [4, 8], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [8, 16], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [12, 24], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [16, 32], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [20, 40], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [24, 48], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [28, 56], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [32, 64], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [40, 80], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [48, 96], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [64, 128], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [84, 168], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [100, 256], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [126, 168], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [128, 256], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [164, 328], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [200, 400], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [256, 512], "kernel_size": 3, "act_cnn": "relu",
     "lstm_hidden_size": 32, "lstm_layers": 2, "optimizer": "adamw", "lr": 1e-2},
]
...
# ============ 4. 数据准备 (核心修改为 80/20 划分, 返回 CV Pool 和 Test Set) ============
def get_data_and_test_set(csv_path, seed, cv_ratio, ts_ratio):
    ...
    # 假设时序数据为 3 个特征，10 个时间步
    X_temporal_all = X_temporal_flat.reshape(-1, 3, 10)
    NUM_CLASSES = len(np.unique(y))
    CLASS_NAMES = [str(i) for i in sorted(np.unique(y))]

    # 第一次划分：分离 80% 的 CV Pool 和 20% 的独立测试集
    # 使用 stratify=y 确保类别比例正确
    X_s_cv, X_s_test, y_cv, y_test = train_test_split(
        X_static_all, y, test_size=ts_ratio, stratify=y, random_state=seed
    )
    X_t_cv, X_t_test, _, _ = train_test_split(
        X_temporal_all, y, test_size=ts_ratio, stratify=y, random_state=seed
    )
    ...
    # 时序数据标准化：将其展平后拟合 StandardScaler
    ...
    print(
        f"数据准备完成。CV Pool: {X_s_cv.shape[0]} 样本 ({cv_ratio * 100:.0f}%) | 独立测试集: {X_s_test.shape[0]} 样本 ({ts_ratio * 100:.0f}%)")
    ...
    return cv_dataset, test_loader, class_weights_t, NUM_CLASSES, CLASS_NAMES, X_s_cv.shape[1]


# 替换原始的调用
cv_dataset, test_loader, class_weights_t, NUM_CLASSES, CLASS_NAMES, NUM_STATIC_FEATS = get_data_and_test_set(
...
class DR_Block_1D(nn.Module):
    def __init__(self, in_features, out_features, kernel_size, activation="relu", dropout=0.3):
        super().__init__()
        self.act_fn = make_activation(activation)
        self.dropout_layer = nn.Dropout(dropout)
        self.conv1 = nn.Conv1d(in_features, out_features, kernel_size, padding='same')
        self.bn1 = nn.BatchNorm1d(out_features)
        self.conv2 = nn.Conv1d(out_features, out_features, kernel_size, padding='same')
        self.bn2 = nn.BatchNorm1d(out_features)
        self.conv3 = nn.Conv1d(out_features, out_features, kernel_size, padding='same')
        self.bn3 = nn.BatchNorm1d(out_features)
        self.conv4 = nn.Conv1d(in_features, out_features, 1, padding='same')
        self.bn4 = nn.BatchNorm1d(out_features)

    def forward(self, x):
        c1 = self.act_fn(self.bn1(self.conv1(x)))
        c2 = self.act_fn(self.bn2(self.conv2(c1)))
        c3 = self.bn3(self.conv3(c2))
        s = self.bn4(self.conv4(x))
        out = self.act_fn(c3 + s)
        return self.dropout_layer(out)


class Hybrid_ANN_DRLSTM_Kernel(nn.Module):
    def __init__(self, num_static_feats, num_classes,
                 widths, act_mlp,
                 cnn_layers, kernel_size, act_cnn,
                 lstm_hidden_size, lstm_layers,
                 final_dropout, mlp_out_features=32, fusion_hidden_features=64
                 ):
        super().__init__()
        ...
        # --- 3. 融合分类器 ---
        combined_features_dim = mlp_out_features + lstm_hidden_size
        self.classifier = nn.Sequential(
            nn.Linear(combined_features_dim, fusion_hidden_features),
            nn.BatchNorm1d(fusion_hidden_features), nn.ReLU(),
            nn.Dropout(final_dropout),
            nn.Linear(fusion_hidden_features, num_classes)
        )

    def forward(self, x_static, x_temporal):
        mlp_features = self.mlp_extractor(x_static)
        # x_temporal 的形状应为 (batch_size, num_features, seq_len)
        cnn_features = self.cnn_extractor(x_temporal)
        # 转换为 (batch_size, seq_len, num_features) 供 LSTM 使用
        lstm_input = cnn_features.permute(0, 2, 1)
        lstm_output, (h_n, c_n) = self.lstm(lstm_input)
        # 取最后一个时间步的输出作为时序特征总结
        temporal_summary = lstm_output[:, -1, :]
        combined_features = torch.cat([mlp_features, temporal_summary], dim=1)
...
def train_and_eval_fold(config, train_indices, val_indices, k_fold_id):
    ...
    # 使用更大的 batch size for evaluation
    val_loader = DataLoader(val_subset, batch_size=BATCH_SIZE_EVAL, shuffle=False)

    # 3. 实例化模型、损失和优化器
    model = Hybrid_ANN_DRLSTM_Kernel(
        num_static_feats=NUM_STATIC_FEATS, num_classes=NUM_CLASSES,
        widths=config["widths"], act_mlp=config["act_mlp"],
    ...
    if best_train_metrics is None:
        # 如果模型从未更新 (数据太少或 PATIENCE=0)，则进行一次最终评估
        val_loader_eval = DataLoader(val_subset, batch_size=BATCH_SIZE_EVAL, shuffle=False)
        y_val_true, _, val_logits = get_preds_and_probs(model, val_loader_eval, device)
    ...
    # 返回当前 Fold 的最佳训练和验证指标
    return best_val_metrics[f"val_{EARLY_MONITOR}"], best_train_metrics, best_val_metrics
...
def train_one_config(config):
    ...
    X_s_cv = cv_dataset.tensors[0].numpy()
    y_cv = cv_dataset.tensors[2].numpy()

    print(f"\n --- 开始 {K_FOLDS}-Fold 交叉验证 ---")

    for fold_id, (train_index, val_index) in enumerate(kf.split(X_s_cv, y_cv)):
...
# --- 核心修改 7.1: 新增 append_plotting_data 函数 ---
...
# --- 核心修改 7.2: 调整 save_plotting_data (仅用于最优模型) ---
...
# --- 核心修改 7.3: 循环训练所有配置的最终模型并评估 (满足用户需求) ---
...
for i, res in enumerate(all_results):
    ...
    # 实例化最终模型 (使用当前配置)
    final_model = Hybrid_ANN_DRLSTM_Kernel(
        num_static_feats=NUM_STATIC_FEATS, num_classes=NUM_CLASSES,
        **model_init_config, final_dropout=FINAL_DROPOUT
    ).to(device)
...
# --- 核心修改 7.4: 最终保存 CSV 文件 (包含所有测试集指标) ---

...
DIST_POWER = 1
# 注意：文件名已修改
SAVE_BEST_PATH = "best_pure_cnn_kfold_model.txt"
RESULTS_CSV = "grid_results_pure_cnn_kfold.csv"
PLOT_DATA_FILE = "plotting_data_pure_cnn_kfold.txt"
# 新增文件: 记录所有配置在独立测试集上的详细绘图数据
ALL_TEST_PLOT_DATA_FILE = "all_test_plotting_data_pure_cnn_kfold.txt"

# ============ 2. GRID_CONFIGS (纯 CNN 配置) ============
# 纯 CNN 模型需要将 CNN 输出扁平化后接全连接分类器
GRID_CONFIGS = [
    # 假设 cnn_layers 是用于定义 CNN 输出通道数的列表

    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [8, 16], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [12, 24], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [16, 32], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [20, 40], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [24, 48], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [28, 56], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [32, 64], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [40, 80], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [48, 96], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [64, 128], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [84, 168], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [100, 200], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [120, 240], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [128, 256], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [164, 328], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [200, 400], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
    {"widths": [64, 32], "act_mlp": "relu", "cnn_layers": [256, 512], "kernel_size": 3, "act_cnn": "gelu",
     "optimizer": "adamw", "lr": 1e-4},
]
...
class Pure_CNN_Model(nn.Module):
    def __init__(self, num_input_channels, num_classes,
                 cnn_layers, kernel_size, act_cnn,
                 final_dropout, widths, act_mlp,  # <--- 接收 ANN 参数
                 sequence_length=10, mlp_out_features=32, fusion_hidden_features=64):
        super().__init__()
        ...
        # 计算 CNN 输出扁平化后的维度
        final_channels = cnn_layers[-1] if cnn_layers else num_input_channels
        cnn_flattened_dim = final_channels * sequence_length  # CNN 时序总结维度

        # --- 3. 融合分类器 ---
        combined_features_dim = cnn_flattened_dim + mlp_out_features  # 融合维度
    ...
    def forward(self, x_static, x_temporal):
        ...
        cnn_flattened = nn.Flatten()(cnn_features)  # (N, CNN_Flattened_Dim)

        # 拼接静态特征和时序特征
        combined_features = torch.cat([static_summary, cnn_flattened], dim=1)
...
# ... (CostSensitiveCE, build_optimizer, get_preds_and_probs, calculate_metrics 保持不变) ...
# ============ 6. 损失/评估/训练函数 (适配 K-Fold) ============
...
def train_and_eval_fold(config, train_indices, val_indices, k_fold_id):
    ...
    # 2. 创建 DataLoader
    train_loader = DataLoader(train_subset, batch_size=BATCH_SIZE_EVAL, shuffle=False)
    ...
    for ep in range(1, MAX_EPOCHS + 1):
        model.train()
        for x_s_b, x_t_b, y_b in train_loader_for_step:  # 使用 shuffle=True 的 loader 进行训练步骤
        ...
        if best_state is None or is_better:
            ...
            # 使用 train_loader (batch_size=EVAL, shuffle=False) 进行快速准确的评估
...
for i, res in enumerate(all_results):
    ...
    # 实例化最终模型 (使用当前配置)
    final_model = Pure_CNN_Model(
        num_input_channels=NUM_INPUT_CHANNELS, num_classes=NUM_CLASSES,
        cnn_layers=model_init_config["cnn_layers"], kernel_size=model_init_config["kernel_size"],
        act_cnn=model_init_config["act_cnn"],
        final_dropout=FINAL_DROPOUT,
        widths=model_init_config["widths"],  # <--- 补充缺失的 ANN 宽度参数
        act_mlp=model_init_config["act_mlp"]  # <--- 补充缺失的 ANN 激活函数参数
    ).to(device)
